...
io.on('connection', async (socket) => {
	...
	socket.on('disconnect', async () => {
		...
		try {
			...
			const playerDetails = await userDetails(customId);

			const gameRoom = await GameDB.findOne({ _id: roomId }, { gameHost: 1 });

			const gameHost = gameRoom.gameHost.toString();

			if (roomData && roomData.length > 0 && roomData[0].players.length > 0) {
				const newHost = roomData[0].players[0];

				if (gameHost == playerDetails._id) {
					await GameDB.findByIdAndUpdate({ _id: roomId }, { $set: { gameHost: new mongoose.Types.ObjectId(newHost) } })
				}
				const newHostDetails = await userDetails(newHost);

				io.in(roomId).emit("roomEvent", { action: "newHost", newHostId: newHostDetails._id.toString(), newHostName: newHostDetails.userName ?? newHostDetails.guestId });
			}
